    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-3 max-w-sm w-full pointer-events-none">
      {toasts.map(toast => {
        ...
        return (
          <div
            key={toast.id}
            className="pointer-events-auto bg-white/95 backdrop-blur-md border-2 border-gray-200 shadow-xl rounded-2xl p-4 flex items-start space-x-3 transition-all transform translate-y-0 animate-bounce-short"
          >
            {icon}
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                <h4 className="font-bold text-gray-900 text-sm truncate">{toast.title}</h4>
