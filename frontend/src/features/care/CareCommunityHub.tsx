...
import { 
  Heart, Building2, UserCheck, Activity, Calendar, Search, MapPin, Phone, Mail, 
  ArrowLeft, Plus, CheckCircle, Volume2, X, Trash2, Clock, Mic
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#FDFBF7] via-[#E8F5E9] to-[#E3F2FD] p-6 md:p-12">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          {/* Tabs */}
          <div className="flex items-center space-x-2 bg-white p-2 rounded-2xl shadow-sm border border-gray-200 overflow-x-auto w-full md:w-auto">
            <button
              onClick={() => setActiveTab('all')}
              className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all whitespace-nowrap ${activeTab === 'all' ? 'bg-[#2E7D32] text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}
            >
              All Support
            </button>
            <button
              onClick={() => setActiveTab('ngo')}
              className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all whitespace-nowrap flex items-center space-x-2 ${activeTab === 'ngo' ? 'bg-[#2E7D32] text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}
            >
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all whitespace-nowrap flex items-center space-x-2 ${activeTab === 'caregiver' ? 'bg-[#2E7D32] text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}
            >
              <UserCheck className="w-4 h-4" />
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all whitespace-nowrap flex items-center space-x-2 ${activeTab === 'rehab' ? 'bg-[#2E7D32] text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}
            >
          ...
          {activeTab !== 'appointments' && (
            <div className="relative w-full md:w-80">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 bg-white border border-gray-200 rounded-2xl shadow-sm text-sm font-medium focus:ring-2 focus:ring-[#2E7D32] focus:outline-none"
              />
            </div>
          )}
        </div>
        ...
        {activeTab === 'appointments' ? (
          ...
          <div className="space-y-4">
            ...
            {appointments.length === 0 ? (
              <GlassCard className="!p-12 text-center text-gray-500 bg-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {appointments.map(app => (
                  <GlassCard key={app.id} className="bg-white border-2 border-amber-200 shadow-md !p-6 flex flex-col justify-between">
                    <div>
                      ...
                      <h4 className="text-xl font-extrabold text-gray-900 mb-1">{app.provider_name}</h4>
                      ...
                      {app.notes && (
                        <p className="text-sm text-gray-600 font-medium bg-gray-50 p-3 rounded-xl border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProviders.map(item => (
              <GlassCard key={item.id} className="bg-white border border-gray-200 shadow-lg !p-6 flex flex-col justify-between hover:shadow-xl transition-shadow">
                <div>
                  {/* Category & Read Aloud */}
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      ...
                      className="p-2 text-gray-400 hover:text-[#2E7D32] hover:bg-gray-100 rounded-full transition-colors"
                  ...
                  <h3 className="text-xl font-extrabold text-gray-900 mb-2 leading-snug">{item.name}</h3>

                  <div className="flex items-center space-x-2 text-sm text-gray-500 font-medium mb-3">
        ...
        {isBookingOpen && selectedProvider && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <GlassCard className="w-full max-w-lg bg-white !p-8 shadow-2xl rounded-3xl relative">
              <button 
              ...
              <form onSubmit={handleCreateAppointment} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Special Requirements / Notes</label>
                  <textarea
                    rows={3}
                    placeholder="Mention specific care needs, patient details, or questions..."
                    value={bookingNotes}
                    onChange={e => setBookingNotes(e.target.value)}
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-[#2E7D32] focus:outline-none resize-none"
                  />
                </div>
